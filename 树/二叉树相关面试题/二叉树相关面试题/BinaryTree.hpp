...
public:
	...
	// 如果有孩子则不是完全二叉树
	// 否则：是
	bool IsCompleteBinaryTree()  //判断二叉树是不是完全二叉树
	{
	...
	{
		int depth = 0;  //用来保存深度（注意：不要使用size_t来保存，因为后面无符号数进行运算，结果还是无符号数）
		return _IsBlance(m_pRoot, depth); //注意:depth是传引用，为了避免调用求高度函数，提高效率
	}
...
private:
	...
	bool _IsCompleteBinaryTree(Node* pRoot); //判断一颗二叉树是否是完全二叉树

	bool _IsBlance(Node* pRoot, int& depth);  //判断一颗二叉树是不是平衡二叉树
...
bool BinaryTree<T>::_IsBlance(Node* pRoot, int& depth)
{
	...
	if (_IsBlance(pRoot->m_pLeft, d_left) && _IsBlance(pRoot->m_pRight, d_right))
	{
		...
		if (diff >= -1 && diff <= 1) //满足左右子树高度相差不大于1
		{
